...
+ (void)queueContentNotifications:(NSArray *)notifications pendingNotifications:(NSMutableArray *)pendingNotifications{
    //Enumerate through the array:
    [notifications enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
    ...
    [[DNDataController sharedInstance] saveContentNotificationsToStore:notifications];
}
...
+ (NSURLSessionTask *)performNetworkTaskForRequest:(DNRequest *)request sessionManager:(DNSessionManager *)sessionManage success:(DNNetworkSuccessBlock)successBlock failure:(DNNetworkFailureBlock)failureBlock {
    ...
    switch ([request method]) {
        case DNPost: {
            ...
            break;
        case DNGet: {
            currentTask = [sessionManage performGetWithRoute:[request route] parameteres:[request parameters] success:^(NSURLSessionDataTask *task, id responseData) {
                if (successBlock)
                    successBlock(task, responseData);
            } failure:^(NSURLSessionDataTask *task, NSError *error) {
                if (failureBlock)
            ...
            break;
        case DNDelete: {
            currentTask = [sessionManage performDeleteWithRoute:[request route] parameteres:[request parameters] success:^(NSURLSessionDataTask *task, id responseData) {
                if (successBlock)
                    successBlock(task, responseData);
            } failure:^(NSURLSessionDataTask *task, NSError *error) {
                if (failureBlock)
            ...
            break;
        case DNPut: {
            currentTask = [sessionManage performPutWithRoute:[request route] parameteres:[request parameters] success:^(NSURLSessionDataTask *task, id responseData) {
                if (successBlock)
                    successBlock(task, responseData);
            } failure:^(NSURLSessionDataTask *task, NSError *error) {
                if (failureBlock)
