...
- (void)sendContentNotifications:(NSArray *)notifications success:(DNNetworkSuccessBlock)successBlock failure:(DNNetworkFailureBlock)failureBlock {
    ...
    NSMutableArray *params = [[DNDataController sharedInstance] sendContentNotifications:notifications];
    [self performSecureDonkyNetworkCall:YES route:kDNNetworkSendNotifications httpMethod:DNPost parameters:params success:successBlock failure:failureBlock];
}
...
- (void)queueContentNotifications:(NSArray *)notifications {
    [DNNetworkHelper queueContentNotifications:notifications pendingNotifications:[self pendingContentNotifications]];
}
