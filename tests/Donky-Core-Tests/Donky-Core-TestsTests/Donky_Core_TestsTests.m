//
//  Donky_Core_TestsTests.m
//  Donky-Core-TestsTests
//
...
@implementation Donky_Core_TestsTests
...
- (void)testGetModule {
    DNModuleDefinition *module = [[DNModuleDefinition alloc]initWithName:@"Unit Tests" version:@"1.0.2.0"];
...
- (void)testUnsubscribeFromNotification {
    DNModuleDefinition *moduleDefinition = [[DNModuleDefinition alloc] initWithName:NSStringFromClass([self class]) version:@"1.0"];
...
- (void)testMultipleModuleSubscriptions {
    ...
    DNModuleDefinition *moduleDefinition2 = [[DNModuleDefinition alloc] initWithName:@"Second Test" version:@"1.0"];
...
- (void)testUpdateUserDetailsIntegration {
    ...
    DNUserDetails *currentUser = [[DNUserDetails alloc] initWithUserID:userID displayName:@"New" emailAddress:@"chris@me.com" mobileNumber:nil countryCode:nil firstName:@"Chris" lastName:@"Watson" avatarID:nil selectedTags:nil additionalProperties:nil];
...
- (void)testUpdateRegistrationDetailsIntegration {
    ...
    NSString *userID = [NSString stringWithFormat:@"UnitTest_%d", arc4random()];
    DNUserDetails *userDetails = [[DNUserDetails alloc] initWithUserID:userID displayName:@"Unit Test" emailAddress:@"chris@me.com" mobileNumber:@"07545804334" countryCode:@"GBR" firstName:nil lastName:nil avatarID:nil selectedTags:nil additionalProperties:nil];
...
- (void)testReplaceRegistrationDetailsAnonymousIntegration {
    ...
    NSString *userID = [NSString stringWithFormat:@"UnitTest_%d", arc4random()];
    DNUserDetails *userDetails = [[DNUserDetails alloc] initWithUserID:userID displayName:@"Display" emailAddress:nil mobileNumber:nil countryCode:nil firstName:nil lastName:nil avatarID:nil selectedTags:nil additionalProperties:nil];
...
- (void)testGetTags {
    ...
    [DNAccountController usersTags:^(NSURLSessionDataTask *task, id responseData) {
        
        NSArray *tags = responseData;
        XCTAssert([tags count]);
        waitingForBlock = NO;
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        XCTFail(@"Failed: %@", [error localizedDescription]);
        waitingForBlock = NO;
...
- (void)testSaveTags {
    ...
    [DNAccountController usersTags:^(NSURLSessionDataTask *task, id responseData) {
        ...
        NSMutableArray *tags = [responseData mutableCopy];
        
        [DNAccountController saveUserTags:tags success:^(NSURLSessionDataTask *task, id responseData) {
            XCTAssertNil(responseData);
            waitingForBlock = NO;
        } failure:^(NSURLSessionDataTask *task, NSError *error) {
            XCTFail(@"Failed: %@", [error localizedDescription]);
